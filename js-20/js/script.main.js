$(function(){
    ...
    $.each(obj, function(i){   
        var skill = _.union(this.skills);
        
        var sort = _.orderBy(skill);
//        console.log(sort);
    ...
    var arr_skills = skills_global.toLocaleLowerCase().split(',');
    arr_skills = _.union(arr_skills);
    arr_skills = _.orderBy(arr_skills);
    arr_skills.splice(0,1);
...
//        $.each(friend, function(i){
//            console.log(i); 
//        });
        
        count_friends = friend.length;
//        console.log(name, count_friends);
        
//        var arr = {'name': name, 'count': count_friends};
         e += (name + ',' + count_friends + ',');

//        e = e.concat(arr);
    });
        var gd = e.split(',');
        gd.splice(length-1,1);
        var name_fields = [
            {'name': gd[0], 'count': gd[1]},
            {'name': gd[2], 'count': gd[3]},
            {'name': gd[4], 'count': gd[5]},
            {'name': gd[6], 'count': gd[7]},
            {'name': gd[8], 'count': gd[9]},
            {'name': gd[10], 'count': gd[11]},
            {'name': gd[12], 'count': gd[13]}
        ];
        var arr_friends = '';
        var result_friends = _.sortBy(name_fields, ['count']);
        $.each(result_friends, function(){
           arr_friends += this.name + ',';
        });
        var arr_result_freinds = arr_friends.split(',');
        arr_result_freinds.splice(length-1,1);
        console.log(arr_result_freinds);
    ...
    $.each(obj, function(){   
        ...
        $.each(friend, function(){
            friend_global += this.name + ',';
//            console.log(friend_global); 
        });
    });
    var arr_friends = friend_global.split(',');
    arr_friends = _.union(arr_friends);
    arr_friends.splice(length-1,1);
    console.log(arr_friends); 
